...
struct socks *
socks_create (const char *host, uint16_t port, const char *u, const char *p)
{
...
struct socks_conn *
socks_accept_conn (struct socks *s)
{
  struct socks_conn *c;
...
int
socks_get_method (struct socks_conn *c)
{
...
int
socks_authenticate (struct socks_conn *c)
{
...
int
socks_command (struct socks_conn *c)
{
  /**
   * +----+-----+-------+------+----------+----------+
...
int
socks_serve (struct socks_conn *c, int fd)
{
  char buf[1024] = { 0 };
  int n;
...
static int
socks_replies (struct socks_conn *c, uint8_t rep, uint8_t atyp,
               const char *addr, int addrlen, uint16_t port)
{
  /**
   * +----+-----+-------+------+----------+----------+
...
static int
socks_ip_connect (struct socks_conn *c, in_addr_t addr, in_port_t port)
{
  struct sockaddr_in si
...
static int
socks_domain_connect (struct socks_conn *c, const char *domain, in_port_t port)
{
